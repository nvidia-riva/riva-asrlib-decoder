...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    ctc_compile_dict_token.sh --extra_word_disamb=${extra_word_disamb} --topo=${topo} --topo_with_sefloops=${topo_with_sefloops} --use_space=${use_space} --space_char=${space_char} --blank_first=${blank_first} ${dict_dir} ${langdir}_tmp ${langdir}
    if [ ! -z ${topo_lm_path} ]; then
...
extend_g_fst.py --word_symbols=${graphdir}/words.txt ${graphdir}/G.fst data/grammar_txt/entities
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    fsttablecompose ${graphdir}/L_disambig.fst ${graphdir}/G.fst.expanded | fstdeterminizestar --use-log=true | \
