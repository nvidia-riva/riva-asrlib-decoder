...
void
WriteCTMOutput(
    std::ostream& ostream, const std::string& key, const kaldi::cuda_decoder::CTMResult& ctm,
    const fst::SymbolTable& word_syms)
{
  ...
  for (size_t iword = 0; iword < ctm.times_seconds.size(); ++iword) {
    ostream << key << " 1 " << ctm.times_seconds[iword].first << ' '
            << (ctm.times_seconds[iword].second - ctm.times_seconds[iword].first) << ' ';
...
main(int argc, char** argv)
{
  ...
  std::string trans_info_rxfilename = po.GetArg(1), fst_rxfilename = po.GetArg(2),
              logits_rspecifier = po.GetArg(3), output_wspecifier = po.GetArg(4),
              ctm_output_path = po.GetArg(5);
  ...
  if (word_syms_rxfilename != "") {
    if (!(word_syms =
              std::unique_ptr<fst::SymbolTable>(fst::SymbolTable::ReadText(word_syms_rxfilename))))
      assert(0 && "Could not read symbol table from file ");
  }

  BatchedMappedDecoderCuda cuda_pipeline(
      batched_decoder_config, *decode_fst, std::move(trans_info), *word_syms);
  ...
  for (; !loglikes_reader.Done(); loglikes_reader.Next()) {
    ...
    float* d_loglikes;
    std::size_t pitch;
    CU_SAFE_CALL(cudaMallocPitch(
        &d_loglikes, &pitch, loglikes.NumCols() * sizeof(float), loglikes.NumRows()));
    // no need for asynchronous copy, as far as I know...
    CU_SAFE_CALL(cudaMemcpy2D(
        d_loglikes, pitch, loglikes.Data(), loglikes.Stride() * sizeof(float),
        loglikes.NumCols() * sizeof(float), loglikes.NumRows(), cudaMemcpyHostToDevice));

    kaldi::Matrix<float> loglikes_copy(loglikes);
    loglikes_copy.SetZero();
    CU_SAFE_CALL(cudaMemcpy2D(
        loglikes_copy.Data(), loglikes_copy.Stride() * sizeof(float), d_loglikes, pitch,
        loglikes_copy.NumCols() * sizeof(float), loglikes_copy.NumRows(), cudaMemcpyDeviceToHost));
    ...
    auto callback = [key, d_loglikes, &output_writer_m, &clat_writer, &ctm_output_path,
      ...
      if (std::get<1>(result).has_value()) {
        Output ctm_output(
            ctm_output_path + "/" + key + ".ctm",
            /*binary=*/false);
        WriteCTMOutput(ctm_output.Stream(), key, std::get<1>(result).value(), word_syms);
      }
    };

    assert(pitch % sizeof(float) == 0);
    cuda_pipeline.DecodeWithCallback(
        d_loglikes, pitch / sizeof(float), loglikes.NumRows(), callback);
  }  // end utterance loop
  ...
  KALDI_LOG << "input_time: " << total_input_time << ", compute_time: " << total_compute_time
