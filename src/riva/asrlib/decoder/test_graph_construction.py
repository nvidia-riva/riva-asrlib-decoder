...
class GraphConstructionTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(lm_zip_file):
            subprocess.check_call(f"wget --content-disposition https://api.ngc.nvidia.com/v2/models/nvidia/tao/speechtotext_english_lm/versions/deployable_v1.0/zip -O {lm_zip_file}", shell=True)
        ...
        if not os.path.exists(am_zip_file):
            subprocess.check_call(f"wget --content-disposition https://api.ngc.nvidia.com/v2/models/nvidia/nemo/stt_en_conformer_ctc_small/versions/1.6.0/zip -O {am_zip_file}", shell=True)
        ...
        subprocess.check_call([os.path.join(riva.asrlib.decoder.__path__[0], "scripts/prepare_TLG_fst/bin/arpa2fst"),
                               f"--write-symbol-table={temp_words_path}",
                               lm_path,
                               "/dev/null"
                               ])
        cls.gzipped_lm_path = lm_path + ".gz"
        with open(lm_path, 'rb') as f_in, \
             gzip.open(cls.gzipped_lm_path, 'wb') as f_out:
            shutil.copyfileobj(f_in, f_out)
                            
        with open(temp_words_path, "r") as words_with_ids_fh, \
             open(cls.words_path, "w") as words_fh:
    ...
    def create_TLG(self, topo, work_dir):
        ...
        subprocess.check_call([prep_subw_lexicon,
                               "--target", "words",
                               "--model_path", self.nemo_model_path,
                               "--vocab", self.words_path,
                               lexicon_path
                               ])
