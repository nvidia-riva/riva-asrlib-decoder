...
CTCTransitionInformation::CTCTransitionInformation(
    const fst::StdFst &ctc_topo) {
  std::unordered_set<int32_t> unique_pdfs;
  for (fst::StateIterator<fst::StdFst> siter(ctc_topo); !siter.Done();
       siter.Next()) {
    fst::StdFst::StateId state_id = siter.Value();
    assert(ctc_topo.Final(state_id) != fst::StdFst::Weight::Zero() &&
           "all states should be final");
    for (fst::ArcIterator<fst::StdFst> aiter(ctc_topo, state_id); !aiter.Done();
         aiter.Next()) {
...
CTCTransitionInformation::CTCTransitionInformation(
    int32_t num_tokens_including_blank)
    : num_pdfs_(num_tokens_including_blank),
      trans_id_to_pdf_id_(num_pdfs_ + 1, -1) {
...
bool CTCTransitionInformation::TransitionIdsEquivalent(
    int32_t trans_id1, int32_t trans_id2) const {
  return trans_id1 == trans_id2; // || trans_id2 == blank_trans_id_;
}

bool CTCTransitionInformation::TransitionIdIsStartOfPhone(
    int32_t trans_id) const {
  return true;
}

int32 CTCTransitionInformation::TransitionIdToPhone(int32 trans_id) const {
  return TransitionIdToPdf(trans_id);
}

bool CTCTransitionInformation::IsFinal(int32 trans_id) const {
  // in a CTC topology, every state should be a final state. We
  // can verify this in the constructor, if necessary.
  return true;
}

bool CTCTransitionInformation::IsSelfLoop(int32 trans_id) const {
  return false;
}
...
int32_t CTCTransitionInformation::NumPdfs() const { return num_pdfs_; }
