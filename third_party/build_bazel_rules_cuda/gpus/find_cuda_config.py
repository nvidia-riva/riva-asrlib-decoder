...
def _get_default_cuda_paths(cuda_version):
    ...
    if _is_windows():
        return [
            os.environ.get("CUDA_PATH", "C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v%s\\" % cuda_version)
        ]
    return [
...
def _find_cuda_config(base_paths, required_version):
    ...
    nvcc_path, nvcc_version = _find_versioned_file(
        base_paths, ["", "bin", "local/cuda/bin",], nvcc_name, cuda_version, get_nvcc_version
    )

    nvvm_path = _find_file(
        base_paths,
        ["nvvm/libdevice", "share/cuda", "lib/nvidia-cuda-toolkit/libdevice", "local/cuda/nvvm/libdevice",],
        "libdevice*.10.bc",
    )
...
def _find_cufft_config(base_paths, required_version, cuda_version):
    ...
    if _at_least_version(cuda_version, "11.0"):
        ...
        def get_header_version(path):
            version = (
                _get_header_version(path, name) for name in ("CUFFT_VER_MAJOR", "CUFFT_VER_MINOR", "CUFFT_VER_PATCH")
            )
            return ".".join(version)
...
def find_cuda_config():
    ...
    base_paths = _list_from_env("CUDA_PATHS", _get_default_cuda_paths(cuda_version))
