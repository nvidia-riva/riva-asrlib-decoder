...
template <> struct type_caster<DLManagedTensor> {
public:
protected:
...
public:
    ...
    bool load(handle src, bool) {
        ...
        if (pybind11::isinstance<pybind11::capsule>(src)) {
            ...
            capsule = pybind11::reinterpret_steal<pybind11::capsule>(PyObject_CallMethod(src.ptr(), "__dlpack__", nullptr));
        } else {
    ...
    static handle cast(DLManagedTensor *src, return_value_policy /* policy */, handle /* parent */) {
        if (src) {
            ...
            pybind11:capsule capsule(src,
                                     reinterpret_cast<void (*)(void*)>(src->deleter));
